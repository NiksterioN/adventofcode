...
// Returns TRUE if a string contains N letters in a row, otherwise returns FALSE
static bool containsNLettersInARow(const std::string &word, const int &n)
{
...
{   
    return containsNVowels(word, 3) && containsNLettersInARow(word, 2) && !containsXXLetters(word);
}
...
static bool testFunctions()
{
    ...
    assert(containsNLettersInARow(wordA, 2) == true);
    assert(containsNLettersInARow(wordC, 2) == false);
